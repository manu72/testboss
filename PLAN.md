...
Comprehensive unit tests will be incorporated at the end of the POC.
...
3.  **`tb step record [--suite <name>] [--env sit] [--session <sessionName>] [--url <startUrl>] [--ai on|off]` Command:**
    ...
    - **Action:** Launch Playwright Codegen in headed mode, pre-configured with the appropriate `baseUrl` or `loginHost` and loaded session storage state.
    - **Action:** Capture raw Playwright actions and locators generated by Codegen as the user interacts with the browser.
    - **Action (AI Integration):** If `--ai on` is specified:
      - Process the raw Codegen output.
      - Call `packages/ai/locator-advisor` to refine Codegen's locators into more durable ones (e.g., prioritizing `data-testid`, `role`).
      - Call `packages/ai/assertion-suggester` to propose one or two low-flake assertions based on the recorded actions and DOM changes.
    - **Action:** Implement an interactive terminal preview using Inquirer.js, allowing the user to review, edit, or accept the refined step and AI suggestions.
